...
const Hero = () => {
  ...
  const randomPopular = popularMovies[7]
    ...
    <div className="w-full h-screen sm:max-h-screen sm:h-[496px] md:h-[500px] lg:h-[550px] xl:h-[600px] relative">
      ...
      <div className="w-full h-full">
        ...
        <div className="absolute top-0 left-0 text-white flex flex-col justify-center items-start h-full xl:gap-10 lg:gap-8 lg:px-28 lg:w-3/4 xl:px-32 xl:w-2/3 md:px-20 md:gap-5 md:w-[70%] sm:px-10 sm:w-[80%] gap-4 px-8">
          <h1 className="lg:text-4xl font-bold xl:text-5xl md:text-2xl text-xl">{randomPopular?.title}</h1>
          <p className="xl:text-lg md:text-sm sm:text-[12px] text-[12px] font-normal md:font-medium">{randomPopular?.overview}</p>
